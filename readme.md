...
- **HTML 正文**：样式化的预测摘要卡片（当前价、预测收盘价、预测涨跌幅、预测区间），
  K 线预测图以 base64 二进制直接内嵌在 HTML 里，正文直接显示图片（`outputs/summary.html`，自包含文件）
- **纯文本回退**：`outputs/summary.txt`，供不支持 HTML 的邮件客户端使用
- **图片附件**：所有股票的 K 线预测图原图（`pred_<代码>_<周期>_chart.png`），方便下载
