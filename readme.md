...
每只股票跑完所有周期的预测后，**立即发送一封该股票的邮件**（邮件标题包含股票代码），例如：

- 标题：`Kronos 股票预测日报 601601 2026-08-09`
- 正文（HTML）：该股票各周期的摘要卡片（当前价、预测收盘价、预测涨跌幅、预测区间），
  K 线预测图以 base64 二进制直接内嵌在 HTML 里，正文直接显示图片，自包含、不引用本地文件

邮件由脚本通过 SMTP 直接发送（每只股票一封），不附加纯文本与图片附件；GitHub Actions 工作流已把
`SMTP_HOST/SMTP_PORT/SMTP_PROTOCOL/SMTP_USER/SMTP_PASS/SMTP_RECIPIENT` 注入到脚本环境。
